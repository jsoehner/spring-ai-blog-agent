...
- **Python CLI Execution in Spring Boot Containers:** When calling python CLI utilities (like `texthumanize.py`) via `ProcessBuilder`, ensure `py3-pip` and required packages are explicitly installed in the runtime stage of multi-stage Dockerfiles (`Dockerfile`), and handle non-zero exit codes gracefully by logging stderr and falling back to original content.
