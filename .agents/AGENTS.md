...
* **TextHumanize Pre-Supervisor Pass Execution:** When integrating `texthumanize` into the blog pipeline, execute text naturalization on the researcher's raw facts *before* sending them to the supervisor blogger agent (`AgentOrchestrator`). Running humanization on factual context before the blogger pass preserves both naturalized text tone and valid WordPress HTML output structure.
